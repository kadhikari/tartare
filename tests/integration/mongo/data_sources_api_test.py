...
def test_post_ds_one_data_source_without_id(app):
    ...
    post_ds = {"name":"data_source_name", "input":{"type":"url"}}
...
def test_post_contrib_one_data_source_without_id(app):
    ...
    post_data["data_sources"].append({"name":"data_source_name", "input":{"type":"url", "url":"stif.com/od.zip"}})
...
def test_post_ds_one_data_source_with_id(app):
    ...
    raw = post(app, '/contributors', json.dumps(post_contrib))
    assert raw.status_code == 201, print(to_json(raw))
    post_ds = {"id": "data_source_id", "name":"data_source_name", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    assert raw.status_code == 201, print(to_json(raw))
...
def test_post_contrib_one_data_source_with_id(app):
    ...
    post_data = {"id": "id_test", "name":"name_test", "data_prefix":"AAA"}
    post_data["data_sources"] = []
    post_data["data_sources"].append({"id": "data_source_id", "name":"data_source_name", "input":{"type":"url"}})
    raw = post(app, '/contributors', json.dumps(post_data))
    assert raw.status_code == 201, print(to_json(raw))
    raw = app.get('/contributors/id_test/')
    r = to_json(raw)
...
def test_post_ds_one_data_source_with_data_format(app):
    ...
    raw = post(app, '/contributors', json.dumps(post_contrib))
    assert raw.status_code == 201, print(to_json(raw))
    post_ds = {"name":"data_source_name", "data_format":"Neptune", "input":{"type":"url", "url": "stif.com/od.zip"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    assert raw.status_code == 201, print(to_json(raw))
    ...
    assert r["data_sources"][0]["input"]["url"] == "stif.com/od.zip"
...
def test_post_contrib_one_data_source_with_data_format(app):
    ...
    post_data = {"id": "id_test", "name":"name_test", "data_prefix":"AAA"}
    post_data["data_sources"] = []
    post_data["data_sources"].append({"name":"data_source_name", "data_format":"Neptune", "input":{"type":"url"}})
    raw = post(app, '/contributors', json.dumps(post_data))
    assert raw.status_code == 201, print(to_json(raw))
    raw = app.get('/contributors/id_test/')
    r = to_json(raw)
...
def test_post_ds_two_data_source(app):
    ...
    raw = post(app, '/contributors', json.dumps(post_contrib))
    assert raw.status_code == 201, print(to_json(raw))
    post_ds = {"name":"data_source_name1", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    assert raw.status_code == 201, print(to_json(raw))
    post_ds = {"name":"data_source_name2", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    assert raw.status_code == 201, print(to_json(raw))
...
def test_post_contrib_two_data_source(app):
    ...
    post_data = {"id": "id_test", "name":"name_test", "data_prefix":"AAA"}
    post_data["data_sources"] = []
    post_data["data_sources"].append({"name":"data_source_name", "input":{"type":"url"}})
    post_data["data_sources"].append({"name":"data_source_name2", "input":{"type":"url"}})
    raw = post(app, '/contributors', json.dumps(post_data))
...
def test_patch_ds_data_source_with_full_contributor(app):
    ...
    raw = post(app, '/contributors', json.dumps(post_contrib))
    assert raw.status_code == 201
    post_ds = {"id": "ds_id", "name":"data_source_name", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    r = to_json(raw)
...
def test_patch_ds_data_source_name_only(app):
    ...
    raw = post(app, '/contributors', json.dumps(post_contrib))
    assert raw.status_code == 201
    post_ds = {"id": "ds_id", "name":"data_source_name", "data_format":"Neptune", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    r = to_json(raw)
    assert raw.status_code == 201, print(r)
...
def test_patch_contrib_data_source_name_only(app):
    ...
    new_data_source["input"] = r["contributors"][0]["data_sources"][0]["input"]
...
def test_patch_ds_one_data_source_name_of_two_and_add_one(app):
    ...
    raw = post(app, '/contributors', json.dumps(post_contrib))
    assert raw.status_code == 201
    post_ds = {"id": "ds1_id", "name":"data_source_name1", "data_format":"Neptune", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    r = to_json(raw)
    assert raw.status_code == 201, print(r)
    post_ds = {"id": "ds2_id", "name":"data_source_name2", "data_format":"Neptune", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    r = to_json(raw)
    assert raw.status_code == 201, print(r)
    ...
    print(r)
    assert raw.status_code == 200, print(r)
    post_ds = {"id": "ds3_id", "name":"data_source_name3", "input":{"type":"url"}}
    raw = post(app, '/contributors/id_test/data_sources', json.dumps(post_ds))
    r = to_json(raw)
    assert raw.status_code == 201, print(r)
...
def test_patch_contrib_one_data_source_name_of_two_and_add_one(app):
    ...
    post_data["data_sources"] = []
    post_data["data_sources"].append({"name":"data_source_name", "data_format":"Neptune", "input":{"type":"url"}})
    post_data["data_sources"].append({"name":"data_source_2", "data_format":"Neptune", "input":{"type":"url"}})
    raw = post(app, '/contributors', json.dumps(post_data))
    r = to_json(raw)
    assert raw.status_code == 201, print(r)
    new_data_source = {}
    ...
    new_data_source["name"] = "name_modified"
    new_data_source["input"] = r["contributors"][0]["data_sources"][1]["input"]
    r["contributors"][0]["data_sources"][0] = new_data_source
    data_source_list = {}
    data_source_list["data_sources"] = [new_data_source, {"name":"data_source_3", "input":{"type":"url"}}]
    print("patching data with ", json.dumps(data_source_list))
    raw = patch(app, '/contributors/id_test', json.dumps(data_source_list))
    r = to_json(raw)
    assert raw.status_code == 200, print(r)
    ...
    assert patched_data_sources[2]["input"]["type"] == "url"
